...
namespace WNA
{
    public class WNA_DamagerBasic : DamageWorker
	{
		...
		private DamageResult ApplyToPawn(DamageInfo dinfo, Pawn pawn)
		{
			...
			{
				EffecterDef effecterDef = ((!damageResult.deflected) ? ((!damageResult.diminishedByMetalArmor) ? EffecterDefOf.DamageDiminished_General : EffecterDefOf.DamageDiminished_Metal) : ((!damageResult.deflectedByMetalArmor || !dinfo.Def.canUseDeflectMetalEffect) ? ((dinfo.Def != DamageDefOf.Bullet) ? EffecterDefOf.Deflect_General : EffecterDefOf.Deflect_General_Bullet) : ((dinfo.Def != DamageDefOf.Bullet) ? EffecterDefOf.Deflect_Metal : EffecterDefOf.Deflect_Metal_Bullet)));
		...
		private void ApplyDamageToPart(DamageInfo dinfo, Pawn pawn, DamageResult result)
		{
			...
			{
				if (pawn.kindDef.race == WNAMainDefOf.WNA_WNThan || pawn.kindDef.race == WNAMainDefOf.WNA_Human)
				{
					result.headshot = true;
				}
				else
				{
					pawn.Kill(new DamageInfo(DamageDefOf.Crush, 1000000f));
				}
			}
