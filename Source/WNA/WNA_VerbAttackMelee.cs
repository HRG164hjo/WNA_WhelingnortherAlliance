...
namespace WNA
{
    internal class WNA_VerbAttackMelee : Verb_MeleeAttack
    {
        private const float RandDamageFactorMin = 0.8f;

        private const float RandDamageFactorMax = 1.2f;
            ...
            if (tool != null && tool.extraMeleeDamages != null)
            {
                foreach (ExtraDamage extraMeleeDamage in tool.extraMeleeDamages)
                {
                    if (Rand.Chance(extraMeleeDamage.chance))
                    {
                        ...
                        num = Rand.Range(num * RandDamageFactorMin, num * RandDamageFactorMax);
        ...
        protected override DamageWorker.DamageResult ApplyMeleeDamageToTarget(LocalTargetInfo target)
        {
            ...
            foreach (DamageInfo item in DamageInfosToApply(target))
            {
                ...
                {
                    result.AddHediff(pawn.health.AddHediff(tool.hediff));
                }
            }
            return result;
        }
